...
public class XianshiSanMa extends Fragment {
...
//			String result = MyUtlis.readFiles(MyConstants.PRODUCT_SANMA,
//					getActivity());
//			return result;
			return DbProductManager.getInstance(context).getProductByCategory("sanma");
		}
...
//		String ad = sp.getString("ad_sanma", "");
//			adBeans = JsonParse.parseAdJson(ad);
		adBeans = DbAdManager.getInstance(context).getAdByCategory("ad_sanma");
		if (adBeans != null) {
	...
	public void onDestroy() {
		super.onDestroy();
		if (flag) {
			DbProductManager.getInstance(context).deleteByCategory("sanma");
			MyUtlis.sortListOrder(mList);
			for (int i = 0; i < mList.size(); i++) {
				if (i <= 20) {
					ProductsBean bean = mList.get(i);
					bean.setCategory("sanma");

...
public class XianshiShouBiaoShouji extends Fragment {
	...
	private void loadButton(View mView) {
		...
		mGridView = (MyGridView) mView.findViewById(R.id.gridview);
 		...
 		mPullRefreshScrollView = (PullToRefreshScrollView) view.findViewById(R.id.pull_refresh_scrollview_td);
...
//			String result = MyUtlis.readFiles(MyConstants.PRODUCT_TD,
//					getActivity());
//			return result;
			 
			return DbProductManager.getInstance(context).getProductByCategory("td");
		}
...
//			}
	...
	private void setAd() {
		...
		AdBean adBean = new AdBean("", MyConstants.PICTURE_URL, "", "", "", "",
				"");
//		String ad = sp.getString("ad_td", "");
//			adBeans = JsonParse.parseAdJson(ad);
		adBeans = DbAdManager.getInstance(context).getAdByCategory("ad_td");
	...
	public void onDestroy() {
		super.onDestroy();
		 if (flag) {
			 DbProductManager.getInstance(context).deleteByCategory("td");
				MyUtlis.sortListOrder(mList);

...
import com.mzhou.merchant.activity.FabuShoujiEnterpriseActivity;
...
public class XianshiLogoProduct extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		View mView = inflater.inflate(R.layout.view_pager_logo_product, null);
		view = mView;
		init();
		loadButton(mView);
		refreshListener();
	...
	private void init() {
		...
		options = new DisplayImageOptions.Builder().showStubImage(R.drawable.ic_stub).showImageForEmptyUri(R.drawable.ic_stub)
				.showImageOnFail(R.drawable.ic_stub).delayBeforeLoading(0).cacheOnDisc()
		...
		mPullRefreshScrollView.setOnRefreshListener(new OnRefreshListener2<ScrollView>() {
			...
			public void onPullUpToRefresh(PullToRefreshBase<ScrollView> refreshView) {
				...
				if (WebIsConnectUtil.showNetState(context)) {
					productsManager.GetProductInfo(getActivity(), classid, page_up, "down");
					productsManager.getProductInfoIml(new IgetProductInfo() {
						@Override
						public void getProductInfo(List<ProductsBean> productsBeans) {
							if ((productsBeans != null) && (!productsBeans.equals("[]"))) {
	...
	public class GetProducts extends AsyncTask<Void, Void, LinkedList<ProductsBean>> {
		...
		protected LinkedList<ProductsBean> doInBackground(Void... params) {
			// String result =
			// MyUtlis.readFiles(MyConstants.PRODUCT_LOGO_PRODUCT,
			// getActivity()); logo_product
			return DbProductManager.getInstance(context).getProductByCategory("logo_product");
		}
	...
	private void setAd() {
		...
		adBeans = DbAdManager.getInstance(context).getAdByCategory("ad_2g3g");
	...
	public void onDestroy() {
		super.onDestroy();
		if (flag) {
			DbProductManager.getInstance(context).deleteByCategory("logo_product");
			MyUtlis.sortListOrder(mList);
			for (int i = 0; i < mList.size(); i++) {
				if (i <= 20) {
					ProductsBean bean = mList.get(i);
					bean.setCategory("logo_product");
